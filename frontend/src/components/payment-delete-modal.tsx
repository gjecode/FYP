...
export default function PaymentDeleteModal({
	...
	// async function to delete payment with payment intent from Payment Service
	const deletePayment = async (paymentID: string) => {
		...
		try {
			...
			const response = await axios.post(
				`http://${orchestratorURL}/deletePayment/`,
				{
					id: paymentID,
				},
				{
			...
			if (response.status === 204) {
				onDelete(paymentID);
				onStatus("Successfully deleted payment!");
			} else {
				onStatus("Failed to delete payment!");
			}
		} catch (error) {
