...
export default function CategoryDeleteModal({
	...
	// async function to delete dog category with category ID from Dogs Service
	const deleteDogCategory = async (categoryID: number) => {
		setDeleting(true);
		onStatus("");
		...
		try {
			...
			const response = await axios.post(
				`http://${orchestratorURL}/deleteDogCategory/`,
				{
					id: categoryID,
				},
				{
			...
			if (response.status === 204) {
				onDelete(categoryID);
				onStatus("Successfully deleted dog category!");
			} else {
				onStatus("Failed to delete dog category!");
			}
		} catch (error) {
			console.error("Delete Dog Category API error:", error);
			handleClose();
			onStatus("Failed to delete dog category!");
		} finally {
			setDeleting(false);
				...
				<Fade in={open}>
					<Box sx={style}>
						...
						<Typography id="transition-modal-description" sx={{ mt: 2 }}>
							Are you sure you want to delete this dog category?
						</Typography>
						<br></br>
						<Stack justifyContent="space-around" direction="row">
							...
							<Button
								onClick={() => deleteDogCategory(categoryID)}
								variant="outlined"
								color="error"
